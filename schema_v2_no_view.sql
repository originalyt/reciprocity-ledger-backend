...
-- =========================================================
-- Reciprocity Ledger v2 (No View + No Foreign Key)
-- 核心思想：
-- 1) 同事件回礼采用 event_exchange 主表落状态
-- 2) 所有关联采用逻辑关联（*_id / code），不使用 FOREIGN KEY
-- =========================================================
...
COMMENT ON COLUMN event_type_dict.code IS '事件类型编码（主键）';
...
COMMENT ON COLUMN event_exchange.id IS '主键ID';
...
COMMENT ON COLUMN ledger_record.id IS '主键ID';
